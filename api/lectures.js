...
export default async function handler(req, res) {
  const { yearId, subjectId, teacherId, chapterId } = req.query;

  if (!yearId || !subjectId || !teacherId || !chapterId) {
    return res.status(400).json({ error: "yearId و subjectId و teacherId و chapterId مطلوبان" });
  }
  ...
  try {
    ...
    const chapter = teacher.chapters.find(c => String(c.id) === String(chapterId) || c.name === chapterId);
    if (!chapter) return res.status(404).json({ error: "الفصل غير موجود" });

    // ✅ هنا نستخدم lectures بدل videos
    const lectures = (chapter.lectures || []).map(lec => ({
      id: lec.id,
      name: lec.name,
      videos_count: lec.videos?.length || 0
    }));

    return res.status(200).json(lectures);

  } catch (err) {
    console.error("Error in /api/lectures:", err);
